...
*   **オートマウスしきい値判定の独自オーバーライド**:
    QMK標準の `AUTO_MOUSE_THRESHOLD` が一部のファームウェアバージョンにおいて動作しない問題（[Keyball Issue #596](https://github.com/Yowkees/keyball/issues/596)）を回避するため、`keymap.c` 側で `auto_mouse_activation` 関数をオーバーライドしました。これにより、大きなしきい値を安全に設定できるだけでなく、**「200ms以上トラックボールの動きがない場合は移動量の累積をリセットする」** 機能を追加し、微少な動きやノイズによる意図しないレイヤー切り替え（誤爆）を防止しました。
